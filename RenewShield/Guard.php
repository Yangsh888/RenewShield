...
class Guard
{
    ...
    public static function boot(): void
    {
        ...
        Basic::enforce($context, $settings);
        ...
        if (!$uaAllowed) {
            $ai = Agent::detectAi($context, $settings);
            if ($ai !== null) {
                self::handleDecision($context, $settings, $ai + [
                    'scope' => 'bot',
                    'score' => 30,
                    'payload' => ['bot_name' => $ai['label'] ?? $ai['name'] ?? 'ai'],
                ]);
            }
        }
        ...
        if (!$uaAllowed && ($settings['blockScriptUa'] ?? '1') === '1') {
            $script = Agent::detectScript($context, $settings);
            if ($script !== null) {
                self::handleDecision($context, $settings, $script + [
                    'scope' => 'bot',
                    'score' => 90,
                    'payload' => ['tool_name' => $script['tool'] ?? 'script'],
                ]);
            }
        }
        ...
        if (
            ...
            && self::isReplayableChallenge($context)
        ) {
        ...
        if (!$uaAllowed && !self::hasPass($context)) {
            $loginAbuse = Abuse::login($context, $settings);
            if ($loginAbuse !== null) {
                self::handleDecision($context, $settings, $loginAbuse + ['scope' => 'login']);
            }

            $writeAbuse = Abuse::write($context, $settings);
            if ($writeAbuse !== null) {
                self::handleDecision($context, $settings, $writeAbuse + ['scope' => 'write']);
            }
        ...
        if (!$uaAllowed) {
            $seq = Seq::evaluate($context, $settings);
            if ($seq !== null) {
    ...
    public static function searchHandle(object $archive, object $_select): void
    {
        $settings = Settings::load();
        if (($settings['enabled'] ?? '0') !== '1' || ($settings['searchProtect'] ?? '1') !== '1') {
            return;
        }
        ...
        self::handleDecision($context, $settings, $issue + ['scope' => 'search']);
    }
...
HTML;
    ...
    public static function comment(array $comment, object $_content): array
    {
        ...
        if ($minSeconds > 0) {
            ...
            $issuedAt = 0;
            if ($token !== '') {
                $payload = self::readToken($token, 'comment', $context, false);
                $expectedContext = self::commentContextFingerprint((int) ($_content->cid ?? 0));
                $tokenContext = (string) ($payload['ctx'] ?? '');
                if ($payload !== null && $tokenContext !== '' && hash_equals($expectedContext, $tokenContext)) {
    ...
    public static function loginSucceed(object $_user, string $name, string $_password, bool $_temporarily, int $_expire): void
    {
        ...
        Abuse::recordLoginSuccess($context);
    ...
    public static function loginFail(object $_user, string $name, string $_password, bool $_temporarily, int $_expire): void
    {
        ...
        Abuse::recordLoginFail($context, $name, $settings);
        self::markBad($context, $settings);
    ...
    private static function risks(Context $context, array $settings): array
    {
        ...
        if (self::hasSmugglingSignals($context)) {
            $risks[] = ['rule' => 'proto.header.invalid', 'score' => 85, 'message' => '检测到请求头冲突或协议字段异常'];
        }
    ...
    {
        if (!self::isReplayableChallenge($context)) {
            self::denyNonReplayable($context, $rule, $score, $message, $payload);
            return;
        }
    ...
    private static function handleDecision(Context $context, array $settings, array $issue): void
    {
        $scope = (string) ($issue['scope'] ?? $context->routeScope());
        $decision = (string) ($issue['decision'] ?? 'observe');
        $rule = (string) ($issue['rule'] ?? 'rule.custom');
        $score = (int) ($issue['score'] ?? 0);
        $message = (string) ($issue['message'] ?? '检测到可疑请求');
        $payload = (array) ($issue['payload'] ?? []);

        if ($decision === 'allow') {
            Log::write($scope, 'allow', 'allow', $rule, $score, $message, $payload);
            return;
        }

        if ($decision === 'observe') {
            self::observe($scope, $rule, $score, $message, $payload);
            return;
        }

        if ($decision === 'block') {
            self::markBad($context, $settings);
            self::block($scope, $rule, $score, $message, $payload);
        }

        if (self::hasPass($context)) {
            self::observe($scope, $rule, $score, '请求已通过基础验证，当前规则仅记录观察', $payload);
            return;
        }
    ...
    private static function commentContextFingerprint(int $contentId): string
    {
        return sha1('comment|' . max(0, $contentId));
    }
    ...
    {
        $remoteIp = $context->remoteIp !== '' ? $context->remoteIp : $context->ip;
        if (self::ipAllowed($remoteIp, (string) ($settings['proxyTrusted'] ?? ''))) {
            return false;
        }
    ...
    private static function denyNonReplayable(Context $context, string $rule, int $score, string $message, array $payload = []): void
    {
        $hint = '当前请求需要先刷新页面后重试，暂不支持在该请求类型上直接完成基础验证';
        $fullMessage = $message !== '' ? ($message . '。' . $hint) : $hint;
        $scope = $context->routeScope();
        Log::write($scope, 'deny', 'block', $rule, $score, $fullMessage, $payload);

        if ($scope === 'comment') {
            throw new \Typecho\Exception($fullMessage);
        }
        ...
        Page::block('请求已被拦截', $fullMessage, Log::payloadSummary($payload));
    }
    ...
    private static function hasSmugglingSignals(Context $context): bool
    {
        ...
        $expect = strtolower((string) ($context->headers['expect'] ?? ''));

        if ($transfer !== '' && $contentLength !== '') {
            return true;
        }

        if ($contentLength !== '' && (str_contains($contentLength, ',') || preg_match('/^\d+$/', $contentLength) !== 1)) {
            return true;
        }

        return $expect !== '' && $expect !== '100-continue';
    }
