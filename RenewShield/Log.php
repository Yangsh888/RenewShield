...
class Log
{
    ...
    public static function payloadSummary(string|array $payload): array
    {
        ...
        if (!empty($decoded['bot_name'])) {
            $items[] = 'Bot：' . (string) $decoded['bot_name'];
        }
        if (!empty($decoded['tool_name'])) {
            $items[] = '工具：' . (string) $decoded['tool_name'];
        }
        if (!empty($decoded['keyword_sample'])) {
            $items[] = '关键词：' . Text::cut((string) $decoded['keyword_sample'], 40);
        }
        if (!empty($decoded['unique_path_count'])) {
            $items[] = '唯一路径：' . (int) $decoded['unique_path_count'];
        }
        if (!empty($decoded['search_mix_count'])) {
            $items[] = '混合类型：' . (int) $decoded['search_mix_count'];
        }
        if (!empty($decoded['attempted_users'])) {
            $items[] = '尝试账号：' . (int) $decoded['attempted_users'];
        }
    ...
    public static function cleanup(int $keepDays): void
    {
        try {
            ...
            $db = Db::get();
            $db->query($db->delete('table.renew_shield_logs')->where('created_at < ?', $before));
        } catch (\Throwable $e) {
    ...
    public static function purge(): void
    {
        try {
            $db = Db::get();
            $db->query($db->delete('table.renew_shield_logs'));
        } catch (\Throwable $e) {
    ...
    private static function ruleCatalog(): array
    {
        return [
            ...
            'agent.ai.qwen' => ['label' => 'AI 爬虫 Qwen', 'group' => 'AI 爬虫', 'hint' => '命中通义相关抓取标识'],
            'agent.ai.amazonbot' => ['label' => 'AI 爬虫 Amazonbot', 'group' => 'AI 爬虫', 'hint' => '命中 Amazonbot 标识'],
            'agent.ai.ccbot' => ['label' => 'AI 爬虫 CCBot', 'group' => 'AI 爬虫', 'hint' => '命中 Common Crawl 相关标识'],
            'agent.script.client' => ['label' => '自动化工具访问', 'group' => 'Bot 识别', 'hint' => '命中 curl、wget、requests、Go-http-client 等工具标识'],
            ...
            'write.rate.limit' => ['label' => '写请求频率超限', 'group' => '写请求防护', 'hint' => 'POST、PUT、PATCH、DELETE 请求在窗口内过多'],
            'write.replay' => ['label' => '写请求重放', 'group' => '写请求防护', 'hint' => '短时间内重复提交相同写请求体'],
            ...
            'search.rate.limit' => ['label' => '搜索频率超限', 'group' => '搜索防护', 'hint' => '搜索请求在窗口内超过阈值'],
            ...
            'login.spray' => ['label' => '疑似撞库喷洒', 'group' => '登录观察', 'hint' => '同一来源在窗口期内尝试多个账号'],
            'login.success' => ['label' => '登录成功', 'group' => '登录观察', 'hint' => '记录一次成功登录'],
            'login.anomaly' => ['label' => '登录环境变化', 'group' => '登录观察', 'hint' => '同一账号出现新的 IP 或 UA 环境'],
            'basic.auth.required' => ['label' => '需要 Basic Auth', 'group' => '基础鉴权', 'hint' => '访问受保护路径但未提供有效 HTTP Basic Auth'],
            'basic.auth.fail' => ['label' => 'Basic Auth 失败', 'group' => '基础鉴权', 'hint' => 'HTTP Basic Auth 用户名或密码错误'],
            'basic.auth.pass' => ['label' => 'Basic Auth 通过', 'group' => '基础鉴权', 'hint' => '受保护路径已通过 HTTP Basic Auth 验证'],
            'sequence.crawl' => ['label' => '低速广撒网访问', 'group' => '行为序列', 'hint' => '页面访问模式表现为广撒网式采集'],
            'sequence.assetless' => ['label' => '只抓页面不抓静态资源', 'group' => '行为序列', 'hint' => '长时间只抓取页面资源而缺少静态资源请求'],
