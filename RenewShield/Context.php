...
class Context
{
    ...
    public static function fromRequest(?Request $request = null): self
    {
        ...
        $self->isSearch = preg_match('#^/search(?:/|$)#i', $self->path) === 1;
    ...
    {
        if ($this->isXmlRpc || $this->isUpload || $this->isShieldAction || $this->isStaticAsset()) {
            return false;
        }
    ...
    {
        return preg_match('/\.(?:css|js|mjs|map|png|jpe?g|gif|webp|svg|ico|woff2?|ttf|eot|mp4|mp3|webm|pdf|txt|xml)$/i', $this->path) === 1;
    }

    public function pathClass(): string
    {
        return match (true) {
            $this->isSearch => 'search',
            preg_match('#^/tag/#i', $this->path) === 1 => 'tag',
            preg_match('#^/category/#i', $this->path) === 1 => 'category',
            preg_match('#^/author/#i', $this->path) === 1 => 'author',
            preg_match('#^/\d{4}/\d{1,2}/#', $this->path) === 1 => 'date',
            preg_match('#/page/\d+/?$#i', $this->path) === 1 => 'page',
            default => 'page',
        };
    }
    ...
    private static function remoteIp(): string
    {
        ...
        return filter_var($remote, FILTER_VALIDATE_IP, FILTER_FLAG_IPV4 | FILTER_FLAG_IPV6) ?: '';
    }
