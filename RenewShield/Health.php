...
class Health
{
    public static function inspect(array $settings): array
    {
        ...
        if (($settings['writeProtect'] ?? '1') !== '1') {
            $message = '当前未启用 POST、PUT、PATCH、DELETE 的专项限频保护。';
            if (($settings['writeReplayCheck'] ?? '1') === '1') {
                $message .= ' 由于写请求保护关闭，body 重放检测当前也不会生效。';
            }
            $items[] = self::item('warn', '写请求防护', $message);
        } elseif (($settings['writeReplayCheck'] ?? '1') !== '1') {
            $items[] = self::item('warn', '请求重放检测', '当前已启用写请求限频，但未启用 body 重放检测。');
        }

        if (($settings['loginIpLimit'] ?? 12) > 30) {
            $items[] = self::item('warn', '登录失败阈值', '同一来源的登录失败次数阈值偏高，可能削弱后台爆破防护效果。');
        }

        if (($settings['loginSprayCheck'] ?? '1') !== '1') {
            $items[] = self::item('warn', '撞库识别', '当前未启用同一来源多账号尝试识别，撞库行为只能依赖普通登录失败阈值。');
        }

        if (($settings['searchProtect'] ?? '1') !== '1') {
            $items[] = self::item('warn', '搜索防滥用', '当前未启用搜索接口限频与关键词枚举防护。');
        }

        if (($settings['aiBotPolicy'] ?? 'observe') === 'allow') {
        ...
        if (($settings['basicAuthEnable'] ?? '0') === '1') {
            if (trim((string) ($settings['basicAuthUser'] ?? '')) === '' || trim((string) ($settings['basicAuthHash'] ?? '')) === '') {
                $items[] = self::item('error', 'Basic Auth', 'HTTP Basic Auth 已开启，但用户名或密码哈希为空，受保护路径将无法正确完成验证。');
            } elseif (trim((string) ($settings['basicAuthRules'] ?? '')) === '') {
                $items[] = self::item('warn', 'Basic Auth', 'HTTP Basic Auth 已开启，但尚未配置任何受保护路径。');
            }
        }

        if (($settings['sequenceProtect'] ?? '0') === '1') {
            if ((int) ($settings['sequenceUniquePathLimit'] ?? 25) < 10 || (int) ($settings['sequenceChallengeScore'] ?? 70) < 40) {
                $items[] = self::item('warn', '行为序列阈值', '行为序列反爬阈值偏低，深度浏览或人工检索时更容易出现误判。');
            }
        } else {
            $items[] = self::item('warn', '行为序列反爬', '当前未启用低速广撒网式访问识别，慢速采集只能依赖通用限频和 WAF 兜底。');
        }
        ...
        if (strlen((string) ($settings['signKey'] ?? '')) < 32) {
            $items[] = self::item('warn', '签名密钥', '挑战验证签名密钥缺失或长度偏短。系统会在读取配置时尝试自动修复；若提示持续存在，请检查插件配置是否可写并重新保存。');
        }
